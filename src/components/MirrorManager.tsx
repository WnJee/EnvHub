...
export const MirrorManager: React.FC<MirrorManagerProps> = ({
  ...
  if (mirrors.length === 0) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-8 text-center text-slate-400 bg-[#090D16]">
        <ShieldAlert className="w-10 h-10 text-slate-600 mb-2.5" />
        <h3 className="text-sm font-semibold text-slate-300">正在读取镜像配置文件...</h3>
      </div>
    );
  }
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-5 bg-[#090D16]">
      {/* Top Banner */}
      <div className="p-4 sm:p-5 rounded-2xl bg-gradient-to-r from-amber-950/30 via-slate-900 to-slate-900 border border-slate-800/80 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-xl">
        <div>
          <div className="flex items-center gap-2 text-amber-400 font-mono text-xs uppercase font-semibold">
            <Zap className="w-3.5 h-3.5" />
            Mirror Acceleration
          </div>
          <h2 className="text-lg sm:text-xl font-bold text-white mt-1">国内镜像源加速与测速</h2>
          <p className="text-xs text-slate-400 mt-0.5 max-w-xl">
            配置与写入 <code className="text-blue-400">~/.npmrc</code>、<code className="text-blue-400">~/.pip/pip.conf</code>、<code className="text-blue-400">~/.cargo/config.toml</code> 与 Go 代理环境变量。
          </p>
        </div>
      ...
      {/* Mirror Tabs */}
      <div className="flex bg-slate-900/80 p-1 rounded-2xl border border-slate-800 gap-1 overflow-x-auto">
        {mirrors.map((m) => {
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="text-xs sm:text-sm font-bold text-slate-200 flex items-center gap-2">
              ...
              {currentMirrorConfig.name} 可用镜像源
            </h3>
            <span className="text-[11px] text-slate-400 font-mono truncate max-w-xs sm:max-w-none">
              当前配置: <span className="text-blue-400 truncate">{currentMirrorConfig.currentMirror}</span>
            </span>
          </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-2.5 shrink-0 justify-end">
                    {/* Ping Badge */}
                    <div
                      className={`px-2 py-0.5 rounded-lg border text-[11px] font-mono font-semibold flex items-center gap-1 ${getPingColor(
                        opt.ping
                      )}`}
                    >
                    ...
                    {/* Switch Button */}
                    {isCurrent ? (
                      <div className="px-3 py-1 rounded-lg bg-emerald-500/10 text-emerald-400 text-xs font-medium flex items-center gap-1 border border-emerald-500/20">
                      ...
                      <button
                        ...
                        className="px-3 py-1 rounded-lg bg-blue-600 hover:bg-blue-500 text-white text-xs font-semibold shadow-sm transition-colors flex items-center gap-1"
                      >
                        <span>应用</span>
